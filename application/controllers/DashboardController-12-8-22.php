	...
	public function saveUserAnswer() {
		...
		if(isset($_POST['genratePdfData']) && !empty($_POST['genratePdfData']) && $_POST['genratePdfData'] == 'saveFinalData'){
			...
			if(!empty($_POST['question_id']) && isset($_POST['question_id'])) {
				...
				$footer_img = '<img src="'.$base64.'" style="height:60px;">'; 
				...
				$canvas->page_text($w-590,25,"HBC", $this->pdf->getFontMetrics()->get_font('helvetica','bold'),12,[0,0,0,"alpha" => 0.6]);

				$canvas->page_text($w-180,25,"Business Growth Report", $this->pdf->getFontMetrics()->get_font('helvetica','bold'),12,[0,0,0,"alpha" => 0.6]);
				...
				if(file_put_contents("./uploads/".$pdfName, $output)) {
					...
					$file1 = $_SERVER["DOCUMENT_ROOT"]."/wbr/software/update-software/uploads/".$pdfName;
